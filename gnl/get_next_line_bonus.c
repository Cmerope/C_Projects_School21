...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/29 10:29:27 by cmerope           #+#    #+#             */
/*   Updated: 2020/12/29 11:19:26 by cmerope          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				get_next_line(int fd, char **line)
{
	int			read_bit;
	char		*buff;
	static char	*static_arr[1024];

	while (line && BUFFER_SIZE > 0 && fd >= 0 && (read_bit = read(fd,
	(buff = malloc(sizeof(char) * (BUFFER_SIZE + 1))), BUFFER_SIZE)) >= 0)
	{
		buff[read_bit] = '\0';
		static_arr[fd] = join_static_arr(&static_arr[fd], buff);
		if ((ft_strchr(static_arr[fd], '\n')))
		{
			if ((static_arr[fd] = copy_and_save(&static_arr[fd], &*line)))
				return (clean(buff) + 1);
		}
		else if ((read_bit == 0) && (ft_strchr(static_arr[fd], '\0')))
		{
			if ((static_arr[fd] = copy_and_save(&static_arr[fd], &*line)))
			{
				static_arr[fd] = NULL;
